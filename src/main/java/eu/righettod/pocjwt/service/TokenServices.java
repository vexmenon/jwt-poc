...
public class TokenServices {
    ...
    public TokenServices() throws IOException, ClassNotFoundException {
        ...
        File f = new File("/home/ripplehireuser12/JwtImplementation/poc-jwt-master/src/main/conf/key-hmac.txt");
        this.keyHMAC = Files.readAllBytes(f.toPath());
        f = new File("/home/ripplehireuser12/JwtImplementation/poc-jwt-master/src/main/conf/key-ciphering.txt");
        this.keyCiphering = Files.readAllBytes(f.toPath());

        //Load issuer ID from configuration text file
        f = new File("/home/ripplehireuser12/JwtImplementation/poc-jwt-master/src/main/conf/issuer-id.txt");
        this.issuerID = Files.readAllLines(f.toPath()).get(0);
    ...
    public Response authenticate(@Context HttpServletRequest request, @Context HttpServletResponse response, @FormParam("login") String login, @FormParam("password") String password) {
        ...
        try {
            ...
            if (Pattern.matches("[a-zA-Z0-9]{1,10}", login)) {
                ...
                Cookie authCookie = new Cookie("AUTH-TOKEN",userFingerprint);
            ...
            if (this.tokenRevoker.isTokenRevoked(cipheredToken)) {
                ...
                if (request.getCookies() != null && request.getCookies().length > 0) {
                    ...
                    for(Cookie cook : cookies){
                        if(cook.getName().equals("AUTH-TOKEN")){
                            userFingerprint = cook.getValue();
                            break;
                        }
                    }
                }
